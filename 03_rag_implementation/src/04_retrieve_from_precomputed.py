...
def retrieve(cfg):
    ...
    df['oracle'] = df['ref']
    df['empty'] = df['ref'].apply(lambda r: [])

    for type in ['sparse', 'dense', 'hybrid', 'oracle', 'empty']:
        logger.info(f'Process {type} retriever results.')

        df[f'{type}_oi'] = df.progress_apply(lambda r: oracle_intersection(r, type, K), axis=1)
        df['retrieved'] = df.progress_apply(lambda r: get_paragraphs(con, r[f'{type}_oi'], K if type != 'oracle' else len(r['ref'])), axis=1)
        save_df(df, f"{cfg.out}/{type}_oi.jsonl")
    
        df['retrieved'] = df.progress_apply(lambda r: get_paragraphs(con, r[f'{type}'], K if type != 'oracle' else len(r['ref'])), axis=1)
        save_df(df, f"{cfg.out}/{type}.jsonl")
