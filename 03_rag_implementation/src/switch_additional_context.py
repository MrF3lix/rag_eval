...
def switch_context(row, input='sparse', output='dense', cutoff=5):
    ref = row['ref']
    ret_input = row[input][:cutoff]
    ret_output = row[output][:cutoff*2] # Worst case is if the number of correct documents in here is >= cutoff => then the switched context will be smaller than cutoff

    correct_documents = list(set(ref) & set(ret_input))
    # incorrect_output_documents = list(set(ret_output) - set(ref))
    incorrect_output_documents = [item for item in ret_output if item not in ref]

    assert len((correct_documents+incorrect_output_documents)[:cutoff]) == cutoff 
    ...
    # print('---')
    # print((correct_documents+incorrect_output_documents)[:cutoff])

    # raise Exception('Done')

    return (correct_documents+incorrect_output_documents)[:cutoff]
...
def retrieve(cfg):
    ...
    df['sparse_p'] = df.progress_apply(lambda r: switch_context(r, 'sparse', 'dense', K), axis=1)
    df['dense_p'] = df.progress_apply(lambda r: switch_context(r, 'dense', 'sparse', K), axis=1)

    df['retrieved'] = df.progress_apply(lambda r: get_paragraphs(con, r['sparse_p'], K), axis=1)
    save_df(df, f"{cfg.out}/sparse_switched.jsonl")

    df['retrieved'] = df.progress_apply(lambda r: get_paragraphs(con, r['dense_p'], K), axis=1)
    save_df(df, f"{cfg.out}/dense_switched.jsonl")
